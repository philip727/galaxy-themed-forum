...
import { createCommentOnPost } from "../../../scripts/api/posts";
...
export default function PostComment({ callComments }: Props) {
    ...
    const handlePostComment = async () => {
        ...
        await postCommentOnPost(comment.current)
...
const postCommentOnPost = async (comment: string) => {
    const [err, result] = await handlePromise<AxiosResponse<any, any>>(createCommentOnPost(comment));
    if (err) {
        createNotification({
